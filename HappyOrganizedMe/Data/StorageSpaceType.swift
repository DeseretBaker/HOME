...
enum StorageSpaceType: String, Codable, CaseIterable, Identifiable, SpaceType {
    ...
    var isCompleted: Bool {
        switch self {
        case .closetSpaces, .builtInSpaces, .longTermStorage: return true
        }
    }
    ...
    var subTaskTypes: [SubTaskTypeBox] {
        switch self {
        case .closetSpaces, .builtInSpaces, .longTermStorage:
            return StorageSubTaskType.allCases.map { SubTaskTypeBox($0)! }
        }
    }
